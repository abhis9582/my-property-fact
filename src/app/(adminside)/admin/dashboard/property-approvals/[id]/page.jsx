...
import { Card, Button, Badge, Spinner, Alert, Modal, Form, Row, Col, Table } from "react-bootstrap";
...
export default function AdminPropertyDetailPage() {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || err.message || "Failed to approve property. Please try again.");
    } finally {
      setProcessing(false);
    }
  };
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || err.message || "Failed to reject property. Please try again.");
    } finally {
      setProcessing(false);
    }
  };
      ...
      <div className="container-fluid py-5">
        <Alert variant="danger">
          ...
          <div className="d-flex gap-2">
            <Button variant="outline-danger" onClick={() => router.push("/admin/dashboard/property-approvals")}>
              Back to Approvals
            </Button>
    ...
    return (
      <div className="container-fluid py-5">
        <Alert variant="warning">
          ...
          <Button variant="outline-primary" onClick={() => router.push("/admin/dashboard/property-approvals")}>
            Back to Approvals
          </Button>
        </Alert>
      </div>
    ...
    <div className="container-fluid py-4 admin-property-detail-page">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
        <div className="d-flex align-items-center gap-3">
          <Button variant="outline-secondary" onClick={() => router.push("/admin/dashboard/property-approvals")}>
      ...
      <Row>
        ...
        <Col lg={8}>
              ...
              <Table bordered>
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>
                      <Badge bg="info" className="me-2">{property.listingType || 'N/A'}</Badge>
                      {property.subType && <Badge bg="secondary">{property.subType}</Badge>}
                    </td>
                  </tr>
                  <tr>
                    <td style={{ fontWeight: '600' }}>Transaction</td>
                    <td><Badge bg="warning">{property.transaction || 'N/A'}</Badge></td>
                  </tr>
                  <tr>
            ...
            <Card.Body>
              <Table bordered>
                <tbody>
                  {property.bedrooms && (
                    <tr>
                      <td style={{ width: '30%', fontWeight: '600' }}>Bedrooms</td>
                  ...
                  {property.ageOfConstruction !== null && property.ageOfConstruction !== undefined && (
                    <tr>
                      ...
                      <td>{property.ageOfConstruction} to {property.ageOfConstruction + 1} Year Old</td>
                    </tr>
                  )}
                  ...
                  {property.noticePeriod !== null && property.noticePeriod !== undefined && (
                    <tr>
                      ...
                      <td>{property.noticePeriod} {property.noticePeriod === 1 ? 'Month' : 'Months'}</td>
                    </tr>
                  )}
                </tbody>
              </Table>
          ...
          <Card className="mb-4">
            ...
            <Card.Body>
              <Table bordered>
                <tbody>
                  <tr>
                    <td style={{ width: '30%', fontWeight: '600' }}>Total Price</td>
                    <td><strong className="text-success">{formatPrice(property.totalPrice)}</strong></td>
                  </tr>
                      ...
                      <div key={benefit.id || index} className="amenity-item">
                        {imageUrl ? (
                          <div className="amenity-image-wrapper">
                            <Image
                              src={imageUrl}
                              alt={benefit.altTag || fullBenefit?.altTag || benefitName}
                              width={60}
                              height={60}
                              className="amenity-image"
                              unoptimized
          ...
          {(property.virtualTour || property.reraId || property.reraState || property.possession || property.occupancy) && (
            <Card className="mb-4">
              <Card.Header>
                ...
                <Table bordered>
                  <tbody>
                    {property.virtualTour && (
                      <tr>
                        <td style={{ width: '30%', fontWeight: '600' }}>Virtual Tour</td>
                        <td>
                ...
                <tbody>
                  ...
                  {property.approvedBy && (
                    <tr>
                      ...
                      <td>{property.approvedBy.email || property.approvedBy.username || 'N/A'}</td>
                    </tr>
                  )}
                  {property.approvedAt && (
                    <tr>
                  ...
                  {property.rejectionReason && (
                    <tr>
                      ...
                      <td className="text-danger">{property.rejectionReason}</td>
                    </tr>
                  )}
                  {property.createdAt && (
                    <tr>
                  ...
                  <tbody>
                    {property.userEmail && (
                      <tr>
                        ...
                        <td>
                          <a href={`mailto:${property.userEmail}`}>{property.userEmail}</a>
                        </td>
                      </tr>
                    )}
                    {property.userPhone && (
                      <tr>
                        ...
                        <td>
                          <a href={`tel:${property.userPhone}`}>{property.userPhone}</a>
                        </td>
                      </tr>
                    )}
                    {property.userId && (
                    ...
                    {property.userRating !== null && property.userRating !== undefined && (
                      <tr>
                        <td style={{ fontWeight: '600' }}>Rating</td>
                        <td>
                          <span className="me-2">{property.userRating.toFixed(1)}</span>
                          <FontAwesomeIcon icon={faStar} className="text-warning" />
                        </td>
                      </tr>
                    )}
                    {property.userTotalDeals !== null && property.userTotalDeals !== undefined && (
                      <tr>
                    ...
                    {property.userCreatedAt && (
                      <tr>
                        ...
                        <td>{new Date(property.userCreatedAt).toLocaleDateString()}</td>
                      </tr>
                    )}
                  </tbody>
                </Table>
      ...
      <Modal show={showRejectModal} onHide={() => setShowRejectModal(false)}>
        ...
        <Modal.Body>
          ...
          {property && (
            <p><strong>{property.title || 'Untitled Property'}</strong></p>
          )}
